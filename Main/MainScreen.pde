public class MainScreen extends Screen
{
  ...
  public void update()
  {
    ...
    for(int i=0; i<6; i++){ // matches 
      ...
      fill(0);
      text("Kamp "+(i+1)+": "+match[i].homeTeam+" - "+match[i].awayTeam+"  "+match[i].weekday+"dag den "+match[i].day+"/"+match[i].month+" "+match[i].year+ " kl: "+match[i].hour+":"+nf(match[i].minute,2), mx+mw/5,my+75*i+mh/2);
    }
    fill(190, 250, 200);
